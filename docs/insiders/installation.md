...
*MkDocs Manpage Insiders* is a compatible drop-in replacement for *MkDocs Manpage*,
...
*MkDocs Manpage Insiders* can be installed with `pip` [using SSH][using ssh]:

```bash
pip install git+ssh://git@github.com/pawamoy-insiders/mkdocs-manpage.git
```
...
```bash
pip install git+https://${GH_TOKEN}@github.com/pawamoy-insiders/mkdocs-manpage.git
```
...
Self-hosting the Insiders package makes it possible to depend on *MkDocs Manpage* normally,
...
with [Twine]:
  ...
  [Github Releases]: https://github.com/pawamoy-insiders/mkdocs-manpage/releases
...
> git clone git@github.com:pawamoy-insiders/mkdocs-manpage
> cd mkdocs-manpage
>
> # install build
...
>
> Now when running `pip install mkdocs-manpage`,
...
Of course, you can use *MkDocs Manpage Insiders* directly from `git`:
...
When upgrading Insiders, you should always check the version of *MkDocs Manpage*
