...
def check_api(ctx: Context) -> None:
    ...
    ctx.run(
        griffe_check("mkdocs_manpage", search_paths=["src"], color=True),
        title="Checking for API breaking changes",
        command="griffe check -ssrc mkdocs_manpage",
        nofail=True,
    )
...
def docs_deploy(ctx: Context) -> None:
    ...
    with material_insiders() as insiders:
        ...
        if "pawamoy-insiders/mkdocs-manpage" in origin:
            ctx.run("git remote add upstream git@github.com:pawamoy/mkdocs-manpage", silent=True, nofail=True)
            ctx.run(
